...
const DEFAULT_FEEDSDB_PATH: &str = "./feeds.db";
...
impl Storage {
  pub fn default() -> Self {
    Self::from_path(std::path::Path::new(DEFAULT_FEEDSDB_PATH))
  }
  ...
  pub fn get_message(&self, msg_id: &str) -> Result<Message> {
    ...
    let raw = db
      .get(Self::key_message(&msg_id))?
      .ok_or(Error::NotFound)?;
    
    let feed_ref = serde_cbor::from_slice::<FeedRef>(&raw)?;
    Ok(self.get_feed(&feed_ref.author, feed_ref.seq_no)?.into_message()?)
  }
  ...
  pub fn append_feed(&self, msg : Message) -> Result<u64>{
    ...
    let feed_ref = serde_cbor::to_vec(&FeedRef{author:author.clone(),seq_no})?;
